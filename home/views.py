    ...
    if User:
        ...
        if name:
            ...
            if College1:
                ...
                # Handle case when College1 is None
                return HttpResponse("College information not found. Please contact the administrator.")
        else:
            # Handle case when name is None
            return HttpResponse("Student information not found. Please contact the administrator.")
...
def execQuiz(request,qName,student_id,qNo,flag):
    ...
    def capture_and_insert_image(student_id, qName):
        while True:
            ...
            combined_frame[webcam_y:webcam_y + resized_webcam_height,
            output_width // 2:output_width // 2 + resized_webcam_width, :] = resized_webcam_frame
...
def execFinal(request, qName, student_id):
    ...
    totalScore = sum(var.total_score for var in resprec)

    totalWarnings = ExamRecord1.warningcount if ExamRecord1 else 0  # Null check
